...
private:
	...
	std::shared_ptr<Value> get_list(List_variables &main_veriables_list)
	{
		std::shared_ptr<Value> rw = main_veriables_list.get_value(this->variable);
		...
		for (int i = 0; i < (*list_expression).get_size() - 1; ++i) {
			rw = (*rw).getElementIndex((*(*(*list_expression).get_element_expression(i))
